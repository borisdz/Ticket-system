...
void BuyTrainTicketView::buttonUpClicked(){
	counter++;
	Unicode::snprintf(ticketCounterBuffer, TICKETCOUNTER_SIZE, "%d", counter);
	ticketCounter.invalidate();
}

void BuyTrainTicketView::buttonDownClicked(){
	counter--;
	Unicode::snprintf(ticketCounterBuffer, TICKETCOUNTER_SIZE, "%d", counter);
	ticketCounter.invalidate();
}
